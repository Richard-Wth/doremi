...
# def get_subdomain_domain_map(ori_domain_list, sub_domain_list):
#     domain_id_map = {}
#     domain_map = {}
#     for subdomain_id, subdomain_name in enumerate(sub_domain_list):
#         for domain_id, domain_name in enumerate(ori_domain_list):
#             if subdomain_name.startswith(domain_name):
#                 domain_map[subdomain_name] = domain_name
#                 domain_id_map[subdomain_id] = domain_id
#     return domain_id_map, domain_map

def get_md_subdomain_domain_map(ori_domain_list, sub_domain_list):
    domain_id_map = {}
    domain_map = {}
    for subdomain_id, subdomain_name in enumerate(sub_domain_list):
        for domain_id, domain_name in enumerate(ori_domain_list):
...
class DoReMiBucketTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        with open(self.args.domain_config_path, 'r') as f:
            self.domain_config = json.load(f)
        ...
        self.train_domain_weights_dict = self.domain_config['train_domain_weights']
        self.eval_domain_weights_dict = self.domain_config['eval_domain_weights']
        self.train_domain_list = list(sorted(self.train_domain_weights_dict.keys()))
        self.eval_domain_list = list(sorted(self.eval_domain_weights_dict.keys()))

        self.eval_domain_list = list(sorted(self.eval_domain_weights_dict.keys()))
        # define the subdomain list and the domain list
        self.domain_list = list(sorted(self.eval_domain_weights_dict.keys()))
        self.subdomain_list = list(sorted(self.train_domain_weights_dict.keys()))
        self.subdomain_to_domain_id, self.subdomain_to_domain_name = get_md_subdomain_domain_map(self.domain_list, self.subdomain_list)
        self.subdomain_sampling_weights = torch.tensor([self.train_domain_weights_dict[subdomain] for subdomain in self.subdomain_list])
    ...
    def write_weights(self, weights):
        self.model.update_counter += 1
        self.model.train_domain_weights[:] = weights.float()
        # 根据上一次更新的 avg_domain_weights 更新本次 avg_domain_weights
        self.model.avg_domain_weights[:] = (self.model.avg_domain_weights * (self.model.update_counter - 1) + weights) / self.model.update_counter
    
    def read_weights(self):
        weights = self.model.train_domain_weights.clone()
        return weights
    ...
    def compute_loss(
        ...
        """
        All models return losses in the first element by default.
        """
        if self.label_smoother is not None and "labels" in inputs:
            labels = inputs.pop("labels")
        else:
    ...
    def update_subdomain_weights(self, scores, scores_mask, subdomain_ids):
        wandb_log_dict = {}
        train_domain_weights = self.read_weights()

        scores = scores.detach()
        subdomain_ids = subdomain_ids.detach()

        if self.args.doremi_optimizer == 'doremiv1':
            domain_scores = {domain: 0.0 for domain in self.domain_list}
            domain_counts = {domain: 0 for domain in self.domain_list}

            for subdomain_id in range(len(self.subdomain_list)):
                subdomain_mask = (subdomain_ids == subdomain_id)
                subdomain_scores_mask = scores_mask[subdomain_mask]
                if subdomain_mask.sum() > 0:
                    curr_subdomain_score = torch.clip(scores[subdomain_mask][subdomain_scores_mask], min=0).mean()
                else:
                    curr_subdomain_score = self.model.perdomain_scores[subdomain_id]
                
                # 映射 subdomain 到 domain
                domain_id = self.subdomain_to_domain_id[subdomain_id]
                domain_name = self.domain_list[int(domain_id)]
                domain_scores[domain_name] += curr_subdomain_score.item()
                domain_counts[domain_name] += 1

                # 记录 subdomain 的权重和分数
                subdomain_name = self.subdomain_list[subdomain_id]
                wandb_log_dict[f"subdomain_weights/{subdomain_name}"] = train_domain_weights[subdomain_id]
                wandb_log_dict[f"subdomain_scores/{subdomain_name}"] = curr_subdomain_score.item()

            # compute the avg score of original domains, the scores needed to be devided by counts since there are 
            # counts number subdomain scores.
            avg_domain_scores = {domain: scores / counts if counts > 0 else 0 
                                for domain, scores in domain_scores.items() 
                                for counts in [domain_counts[domain]]}
            
            # update weights of original domains
            # log_new_domain_weights = torch.log(torch.tensor([train_domain_weights[self.subdomain_list.index(sd)] 
            #                                                 for sd in self.subdomain_list 
            #                                                 if self.subdomain_to_domain_name[sd] == domain 
            #                                                 for domain in self.domain_list]))
            log_new_domain_weights = torch.log(torch.tensor([
                                                            sum(
                                                                train_domain_weights[self.subdomain_list.index(sd)]
                                                                for sd in self.subdomain_list
                                                                if self.subdomain_to_domain_name[sd] == domain
                                                            )
                                                            for domain in self.domain_list
                                                        ]))
            log_new_domain_weights += self.args.reweight_eta * torch.tensor([avg_domain_scores[domain] for domain in self.domain_list])
            log_new_domain_weights = log_new_domain_weights - torch.logsumexp(log_new_domain_weights, dim=0)
            new_domain_weights = (1-self.args.reweight_eps) * torch.exp(log_new_domain_weights) + self.args.reweight_eps / len(log_new_domain_weights)

            # Assign new domain weights to subdomains, we assign domain's weights to subdomain
            # new_subdomain_weights = torch.zeros_like(train_domain_weights)
            # for i, subdomain in enumerate(self.subdomain_list):
            #     domain = self.subdomain_to_domain_name[subdomain]
            #     domain_id = self.domain_list.index(domain)
            #     new_subdomain_weights[i] = new_domain_weights[domain_id]

            # modify the weights assign machine
            new_subdomain_weights = torch.zeros_like(train_domain_weights)
            for domain_id, domain_name in enumerate(self.domain_list):
                related_subdomains = [sd for sd in self.subdomain_list if self.subdomain_to_domain_name[sd] == domain_name]
                total_weight = sum(train_domain_weights[self.subdomain_list.index(sd)] for sd in related_subdomains)

                if total_weight == 0:
                    subdomain_count = len(related_subdomains)
                    new_subdomain_weights[self.subdomain_list.index(sd)] = new_domain_weights[domain_id] / subdomain_count
                else:
                    for sd in related_subdomains:
                        sd_index = self.subdomain_list.index(sd)
                        weight_ratio = train_domain_weights[sd_index] / total_weight
                        new_subdomain_weights[sd_index] = new_domain_weights[domain_id] * weight_ratio

            self.write_weights(new_subdomain_weights)

            # 记录原始域的权重和分数
            for domain_id, domain in enumerate(self.domain_list):
                wandb_log_dict[f'domain_weights/{domain}'] = new_domain_weights[domain_id].item()
                wandb_log_dict[f'domain_scores/{domain}'] = avg_domain_scores[domain]

        wandb_log_dict['max_subdomain_id'] = subdomain_ids.max().item()
        wandb.log(wandb_log_dict, commit=False)

    def update_subdomain_weights_directly(self, scores, scores_mask, subdomain_ids):
        wandb_log_dict = {}
        subdomain_weights = self.read_weights()
        # print(subdomain_ids)

        scores = scores.detach()
        subdomain_ids = subdomain_ids.detach()
        # print(self.subdomain_list)

        if self.args.doremi_optimizer == "doremiv1":
            subdomain_scores = torch.zeros(len(self.subdomain_list)).to(scores.device)
            for subdomain_id in range(len(self.subdomain_list)):
                subdomain_mask = (subdomain_ids == subdomain_id)
                subdomain_scores_mask = scores_mask[subdomain_mask]
                if subdomain_mask.sum() > 0:
                    subdomain_scores[subdomain_id] = torch.clip(scores[subdomain_mask][subdomain_scores_mask], min=0).mean()
                else:
                    subdomain_scores[subdomain_id] = self.model.perdomain_scores[subdomain_id]
            # print(subdomain_scores)
            self.model.perdomain_scores[:] = subdomain_scores.float()
            # update subdomain weights directly
            log_new_subdomain_weights = torch.log(subdomain_weights)
            log_new_subdomain_weights += self.args.reweight_eta * subdomain_scores
            log_new_subdomain_weights = log_new_subdomain_weights - torch.logsumexp(log_new_subdomain_weights, dim=0)
            new_subdomain_weights = (1-self.args.reweight_eps) * torch.exp(log_new_subdomain_weights) + self.args.reweight_eps / len(log_new_subdomain_weights)
            self.write_weights(new_subdomain_weights)
        else:
            raise ValueError(
        ...
        # log subdomain weights and scores
        for subdomain_id, subdomain_name in enumerate(self.subdomain_list):
            wandb_log_dict[f'subdomain_weights/{subdomain_name}'] = new_subdomain_weights[subdomain_id].item()
            wandb_log_dict[f'subdomain_scores/{subdomain_name}'] = subdomain_scores[subdomain_id].item()

        # log domain weights and scores, optional
        domain_weights = {}
        domain_scores = {}
        for domain_idx, domain in enumerate(self.domain_list):
            related_subdomains = [sd for sd in self.subdomain_list if self.subdomain_to_domain_name[sd] == domain]
            related_subdomain_indices = [self.subdomain_list.index(sd) for sd in related_subdomains]
            
            domain_weights[domain_idx] = sum(new_subdomain_weights[i] for i in related_subdomain_indices)
            domain_scores[domain_idx] = sum(subdomain_scores[i] for i in related_subdomain_indices) / len(related_subdomains)
            wandb_log_dict[f'domain_weights/{domain}'] = domain_weights[domain_idx].item()
            wandb_log_dict[f'domain_scores/{domain}'] = domain_scores[domain_idx].item()
    ...
    def training_step(self, model, inputs):
        ...
        if self.args.reweight_domains:
            ...
            if self.is_local_process_zero():
                gathered_excess_loss = [
                    torch.zeros_like(excess_loss) for _ in range(self.args.world_size)
                ]
                dist.gather(excess_loss, gathered_excess_loss, dst=0)
                gathered_excess_loss = torch.cat(gathered_excess_loss, dim=0)
                ...
                gathered_subdomain_id = [
                    torch.zeros_like(inputs['domain_ids']) for _ in range(self.args.world_size)
                ]
                dist.gather(inputs['domain_ids'], gathered_subdomain_id, dst=0)
                gathered_subdomain_id = torch.cat(gathered_subdomain_id, dim=0)
                
                self.pertoken_scores.append(gathered_excess_loss.detach())
                self.token_masks.append(gathered_token_mask.detach())
                self.subdomain_ids.append(gathered_subdomain_id.detach())

                if len(self.pertoken_scores) == self.args.gradient_accumulation_steps:
                    pertoken_scores = torch.cat(self.pertoken_scores, dim=0)
                    token_masks = torch.cat(self.token_masks, dim=0).bool()
                    subdomain_ids = torch.cat(self.subdomain_ids, dim=0)

                    # update subdomain weights
                    # you can update subdomain weights based on domain weights
                    # you also can update subdomain weights directly
                    # we use the second choice to update subdomain weights
                    self.update_subdomain_weights_directly(
                        pertoken_scores,
                        token_masks,
                        subdomain_ids
                    )
            ...
            if self.args.doremi_optimizer == 'doremiv1':
                # compute the rescaled loss, divided by doamin weights
                train_subdomain_weights = self.read_weights().to(pertoken_loss.device).float()

                # if doing non-uniform sampling, normalize by inverse sampling weights
                train_subdomain_weights = train_subdomain_weights / self.subdomain_sampling_weights.to(train_subdomain_weights.device)
                train_subdomain_weights = train_subdomain_weights / train_subdomain_weights.sum()
                curr_subdomain_weights = train_subdomain_weights[inputs['domain_ids']].unsqueeze(-1).expand_as(pertoken_loss).detach()

                curr_subdomain_weights = curr_subdomain_weights + token_mask

                # renormalize
                normalizer = curr_subdomain_weights.detach().sum()
                # gather normalizer across GPUs
                dist.all_reduce(normalizer, op=torch.distributed.ReduceOp.SUM)
                # scale by world size because DDP averages gradients
                normalizer = torch.clip(normalizer, min=1e-10) / self.args.world_size
                ...
                token_mask = token_mask.detach().type(pertoken_loss.dtype)
                loss = (pertoken_loss * curr_subdomain_weights.detach()).sum() / normalizer
            else:
                raise ValueError(f"doremi_optimizer {self.args.doremi_optimizer} is not supported")
        else:
            with self.compute_loss_context_manager():
    ...
    def load_checkpoint(self, resume_from_checkpoint=None):
        ...
        # Load prtential model checkpoint
        if resume_from_checkpoint is None:
            resume_from_checkpoint = get_last_checkpoint(self.args.output_dir)
    ...
    def evaluation_loop(
        ...
        """
        Prediction/evaluation loop, shared by 'Trainer.evaluate()' and 'Trianer.predict()'.
        ...
        logger.info(f"***** Running {description} *****")
        if has_length(dataloader):
            logger.info(f"Num example = {self.num_examples(dataloader)}")
        else:
            logger.info("Num example: Unknown")
        logger.info(f"Batch size. = {batch_size}")
        ...
        for step, inputs in tqdm(enumerate(dataloader)):
            ...
            subdomain_ids = inputs["domain_ids"].to(loss.device)
            # map subdomain ids to domain ids
            domain_ids = torch.tensor([self.subdomain_to_domain_id.get(sid.item(), -1) for sid in subdomain_ids], device=subdomain_ids.device)
            ...
            #comput losses per domain
            for domain_idx, domain_name in enumerate(self.eval_domain_list):
                domain_mask = (domain_idx == domain_ids)
                examplecounts[domain_idx] = examplecounts[domain_idx] + domain_mask.sum()
