"""
Filter the Pile data into subdomains and tokenize the data.
"""
...
PILE_SUBDOMAINS = []
for i in range(10):
    PILE_SUBDOMAINS.append(str(i+1))
...
def main():
    ...
    parser.add_argument("--subdomain", type=str, default="1")
    ...
    # Count total examples in the domain
    total_examples = sum(1 for _ in ds)

    # Create subdomain assigner
    random.seed(args.seed)
    subdomain_counts = defaultdict(int)

    # Balance data point number in every subdomains
    def assign_subdomain():
        nonlocal subdomain_counts
        if sum(subdomain_counts.values()) == total_examples:
            return random.choice(PILE_SUBDOMAINS)
        elif min(subdomain_counts.values()) < total_examples // len(PILE_SUBDOMAINS):
            return min(subdomain_counts, key=subdomain_counts.get)
        else:
            available_subdomains = [s for s in PILE_SUBDOMAINS if subdomain_counts[s] < (total_examples // len(PILE_SUBDOMAINS)) + 1]
            return random.choice(available_subdomains)

    def data_generator():
        for ex in ds:
            subdomain = assign_subdomain()
            subdomain_counts[subdomain] += 1
            ex['domain_id'] = DOMAIN_TO_IDX[args.domain]
            ex['subdomain_id'] = SUBDOMAIN_TO_IDX[subdomain]
            yield ex
    ...
    # Save dataset
    if args.split == "train":
        intermediate_dir = Path(args.intermediate_dir) / args.split / args.domain / args.subdomain / args.subset
    else:
        intermediate_dir = Path(args.intermediate_dir) / args.split / args.domain / args.subdomain
