...
source /home/wth/My_codes/doremi/scripts/multi_domain/constant.sh
...
REFERENCE_WEIGHTS_NAME=${2:-"md_baseline_50kvocab_nopack"}
# name of the reference model to load (can be different from reference weights in later rounds)
REFERENCE_MODEL_NAME=${3:-"md_baseline_50kvocab_nopack"}
OTHER_ARGS=$4

NAME=md_doremi_r${ROUND}_120M_ref:${REFERENCE_WEIGHTS_NAME}_120M
accelerate launch \
    --config_file accelerate_config.yml \
    --num_processes 8 \
    --num_machines 1 \
    --main_process_port 60667 \
    ...
    --do_train \
    --cache_dir ${CACHE} \
    --dataset_dir ${PREPROCESSED_MD_DIR} \
    --domain_config_path /home/wth/My_codes/doremi/configs/md_uniform.json \
