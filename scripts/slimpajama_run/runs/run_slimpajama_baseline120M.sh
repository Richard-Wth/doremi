...
DOMAIN_CONFIG_NAME=${1:-slim_baseline_50kvocab_nopack}
arg=${2:-""} # set to eval to run eval

if [[ "${arg}" == "eval" ]]; then
   ADDITIONAL_ARGS="--evaluation_strategy steps --per_device_eval_batch_size 256 --do_train false --remove_unused_columns=False --downstream_datasets trivia_qa,web_questions,natural_questions,lambada,squad_v2 --eval_all_checkpoints --skip_perplexity_eval"
else
    ADDITIONAL_ARGS=""
fi
...
accelerate launch \
    --config_file /home/wth/My_codes/doremi/scripts/slimpajama_run/runs/accelerate_config.yml \
    --num_machines 1 \
    --num_processes 8 \
    --multi_gpu \
    --main_process_port 60128 \
    ...
    --domain_config_path /home/wth/My_codes/doremi/configs/${DOMAIN_CONFIG_NAME}.json \
